...
class InputSlotBackgroundPainter(private val itemSlot: WItemSlot, private val texture: Identifier) : BackgroundPainter {
    override fun paintBackground(matrices: MatrixStack?, left: Int, top: Int, panel: WWidget?) {
        ...
        ScreenDrawing.texturedRect(matrices, left, top, itemSlot.width, itemSlot.height, texture, Colors.TRANSPARENT.toRgb())
    }
}
...
class PuzzleComposerScreenDescription(
    ...
    init {
        ...
        outputSlot.addChangeListener { _, inventory, index, changedItemStack ->
        ...
        editor.setClickListener { node, edge, edgeNodePair ->
            ...
            val inputNbt: NbtCompound = inputStack.nbt ?: return@setClickListener
            val inputPanel: Panel? = inputNbt.getPanel(KEY_PANEL)
        ...
        layout()
        context?.run { world, _ -> if (world.isClient) addPainters() }
    }
